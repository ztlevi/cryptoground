...
router.post('/', function(req, res, next) {
  firebase.verifyIdToken(req.body.idToken).then(uid => {
    ...
    firebase.fetchUserBalance(uid).then(response => {
      if (!response) res.send('Fetching user balance failed!!!');
      let verifyResult = verifyTradingRequest(req, balance);
      if (verifyResult.status === 'complete') {
        updateUserBalance(uid, verifyResult.balance).then(response => {
          if (!response)
